...
?>
   <style>
      ...
      .listProdTitle{
         font-size: 18px;
         max-width: 250px; /* limit title width to the same width of the of the image */
      }
      .listProdCat{
         max-width: 250px;
      }
   ...
   <?php if ($search_option == 'Products' || $search_option == $selected || $search_option == $selected): ?>
      <form method="get" action="displayResults.php">
        ...
        <!-- Buttons -->
        <div class="rowcontainer-fluid"><br>
            <div class="col1">
               ...
               <a href="displayResults.php"><button type="button" class="btn btn-outline-secondary my-2 my-sm-0" onclick="resetFilters()">Reset Filter</button></a>
            </div> 
         </div>
    </form>
    ...
    <div class="container-fluid">
      <div class="row">      
         <p><?php if (isGetRequest()){if (isset($_GET["search"])){echo 'Results for: '. $listProdCat .' '. $listCond .' '.'<b>' .$listProdTitle .'</b>';}} ?></p>
      </div>    
   </div> 
   ...
   <div class="container">      
      ...
      <?php foreach ($listArray as $row): ?> 
         <div class="col-sm content">
            ...
            <div class="listState"><?= $row['listState']; ?></div>
            <a href="productDetails.php?listID=<?=$row['listID'];?>">
               <div class="listProdPic"><img src="<?= $row['listProdPic']; ?>" style="object-fit: contain; object-position: center; width: 230px; height: 230px; background-color: #F6F6F6;"></div>
               <div class="listProdTitle"><?= $row['listProdTitle']; ?></div>
            </a>
            <div class="listProdCat"><?= $row['listProdCat']; ?></div>
            <div class="listProdPrice">$<?= $row['listProdPrice']; ?></div>
            <div class="listCond"><?= $row['listCond']; ?></div>
         </div>
         <?php endforeach;?>
      </div>
   </div>
